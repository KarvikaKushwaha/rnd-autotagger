...
for i, row in df.iterrows():
    ...
    try:
        ...
        logging.error("familyid not found for given md5")
    ...
    if len(response['hits']['hits']) > 0:
        for i in range(len(response['hits']['hits'])):
            hit = response['hits']['hits'][i]['_source']
            try:
                if hit['md5hash'] not in style_present:
                    print(hit['md5hash'], hit['stylename'])
                    missing_list.append([hit['md5hash'],
